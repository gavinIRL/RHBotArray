...
class PlayerPositionTest():
    ...
    def start(self):
        ...
        # set the required
        xreqd = 40
        # Then figure out how long to press key
        time_reqd = xreqd/self.speed
        self.press_key(self.key_map["right"])
        time.sleep(time_reqd-0.003)
        self.release_key(self.key_map["right"])
        player_pos_end = self.grab_player_pos()
        print(player_pos_end)
        x_travel = player_pos_end[0]-player_pos[0]
        accuracy = x_travel/xreqd
